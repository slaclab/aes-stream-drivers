...
run_tuser_case() {
   ...
   # Drain stale frames from previous tests: run briefly with target
   # fuser/luser; any leftover frames with old values will cause a
   # Read Error in this throw-away run, clearing the queue.
   timeout 1 "$APP_BIN/dmaLoopTest" -p "$DEV" -m 0 -s "$SIZE" -f "$fuser" -l "$luser" > /dev/null 2>&1 || true
   sleep 1
