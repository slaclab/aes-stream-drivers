...
const  char * argp_program_version = "pgpWrite 1.0";
...
static struct PrgArgs DefArgs = { "/dev/pgpcard_0", 0, 0, 1000, 1, 0, 0 };

static char   args_doc[] = "lane vc";
static char   doc[]      = "   Lane and VC are passed as integers.";
   ...
   switch (key) {
      ...
      case ARGP_KEY_ARG:
          switch (state->arg_num) {
             case 0: args->lane = strtol(arg, NULL, 10); break;
             case 1: args->vc   = strtol(arg, NULL, 10); break;
...
int main(int argc, char **argv) {
   ...
   if (((1 << args.lane) & info.laneMask) == 0) {
      printf("Invalid lane %i. Card laneMask = 0x%.2x\n", args.lane, info.laneMask);
      return(1);
   }
   ...
   do {
      ...
      if (ret <= 0) {
         ...
         // DMA Write
         if (args.idxEn) {
            ret = dmaWriteIndex(s, dmaIndex, args.size, 0, pgpSetDest(args.lane, args.vc));
         } else {
            ret = dmaWrite(s, txData, args.size, 0, pgpSetDest(args.lane, args.vc));
         }

         if (ret > 0) {
            prbValid = false;
            count++;
            printf("Write ret=%i, Lane=%i, Vc=%i, count=%i\n", ret, args.lane, args.vc, count);
         }
      }
