...
const  char * argp_program_version = "pgpSetLoop 1.0";
...
static struct PrgArgs DefArgs = { "/dev/pgpcard_0", 0xFF, 0x00 };

static char   args_doc[] = "enable";
static char   doc[]      = "\n   Enable is either 0 or 1.";
...
int main (int argc, char **argv) {
   ...
   for (x=0; x < 8; x++) {
      if ( ((1 << x) & info.laneMask) != 0 ) {
         printf("Setting lane %i loop to %i\n",x,args.state);
         pgpSetLoop(s,x,args.state);
      }
   }
   close(s);
}
