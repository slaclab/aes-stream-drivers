...
void *runWrite(void *t) {
   ...
   while (txData->enable) {
      ...
      if (ret != 0) {
         ...
         if (txData->idxEn) {
            ret = dmaWriteIndex(fd, dmaIndex, txData->size, 0, pgpSetDest(lane, vc));
         } else {
            ret = dmaWrite(fd, data, txData->size, 0, pgpSetDest(lane, vc));
         }
   ...
   while (rxData->enable) {
      ...
      if (ret != 0) {
         if (idxEn) {
            ...
            ret = dmaRead(fd, data, maxSize, NULL, &rxErr, &rxDest);
         }
...
int main(int argc, char **argv) {
   ...
   for (x = 0; x < 32; x++) {
      if (((1 << x) & args.vcMask) && ((1 << (x/4)) & info.laneMask) && ((1 << (x%4)) & info.vcPerMask)) {
         ...
         snprintf(rxData[vcCount]->id, sizeof(rxData[vcCount]->id), "%i-%i", x/4, x%4);
