...
# -----------------------------------------------------------------------------
# Description:
#    DKMS GPU smoke test. Builds GPU tarball, loads it into the DKMS tree
...
echo_step "DKMS GPU smoke test (tarball + add only)"
...
if [ -f data_dev/driver/datadev-gpu-dkms-${GITV}.tar.gz ]; then
   echo "Loading DKMS GPU tarball..."
   dkms ldtarball data_dev/driver/datadev-gpu-dkms-${GITV}.tar.gz

   # Verify the package was registered
   dkms status datadev-gpu-dkms/${GITV} | grep -q "added" || {
      echo_fail "DKMS GPU package not registered after ldtarball"
      dkms status
      exit 1
   }

   echo "PASS: DKMS GPU tarball registered successfully"
...
echo_step "DKMS GPU smoke test passed"
